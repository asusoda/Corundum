...
public class BlockType extends HoldableType<BlockType> {
    ...
    public static final BlockType AIR = new BlockType(0, -1),
            ...
            YELLOW_CARPET = new BlockType(), LIGHT_GREEN_CARPET = new BlockType(), PINK_CARPET = new BlockType(), DARK_GRAY_CARPET = new BlockType(),
            LIGHT_GRAY_CARPET = new BlockType(), CYAN_CARPET = new BlockType(), PURPLE_CARPET = new BlockType(),
            ...
            // large plants
            SUNFLOWER = new BlockType(0), LILAC = new BlockType(), DOUBLE_TALL_GRASS = new BlockType(),
            ...
            // END red sandstone
            RED_SANDSTONE_STAIRS = new BlockType(), DOUBLE_RED_SANDSTONE_SLAB = new BlockType(), RED_SANDSTONE_SLAB = new BlockType(), SPRUCE_FENCE_GATE = new BlockType(),
            BIRCH_FENCE_GATE = new BlockType(), JUNGLE_FENCE_GATE = new BlockType(), DARK_OAK_FENCE_GATE = new BlockType(), ACACIA_FENCE_GATE = new BlockType(),
            SPRUCE_FENCE = new BlockType(), BIRCH_FENCE = new BlockType(), JUNGLE_FENCE = new BlockType(), DARK_OAK_FENCE = new BlockType(), ACACIA_FENCE = new BlockType(),
            SPRUCE_DOOR = new BlockType(), BIRCH_DOOR = new BlockType(), JUNGLE_DOOR = new BlockType(), ACACIA_DOOR = new BlockType(), DARK_OAK_DOOR = new BlockType();
     ...
     * - <tt>Material.getCanBurn</tt>: tells if fire can burn the block or not; wood-based things can, stone-based things can't; TODO: what about grass blocks that change to
     * dirt? <br>
     ...
     * !blocksMovement <br>
     * - <tt>Material.isToolNotRequired</tt>: tells whether or not a tool is required to acquire drops from this; +: dirt, sand, and signs; -: stone, cobblestone, and brewing
     * stands <br>
     * - <tt>Material.isAdventureModeExempt</tt>: true if it can always be mined in Adventure Mode <br>
     * - <tt>Material.getCanBlockGrass</tt>: tells if the presence of this block atop a dirt block can stop grass from spreading to that dirt; TODO: why does block take the !
     * of it? <br>
     * <b>int</b>s: <br>
     * - <tt>Material.getMaterialMobility</tt>: 0 for most blocks (e.g. dirt), 1 if it cannot push other blocks, but pistons can push over them (e.g. torches), 2 if it cannot
     * be pushed at all (e.g. bedrock) <br>
     * - <tt>Block.getLightOpacity</tt>: the amount of light lost going through this block; 0 for anvils and enchantment tables, 1 for leaves and webs, 3 for water and ice,
     * 255 otherwise <br>
     * <b>float</b>s: <br>
    ...
    // constructors
    /** This constructor makes a BlockType based on the previous value's I.D. and data. If the previous value has no strictly associated data value (data value = -1), it means
     * that it has no sub-types (e.g. the different colors of wool or types of wood), so use the next I.D.; if it has a data value, give this BlockType the same I.D. and the
     * next data value. Essentially, "I.D. blocks" (blocks of multiple enum constants that all have the same I.D., but different data values) are delimited by the use of the
     * {@link #BlockType(int)} and {@link #BlockType(int, int)} constructors; declaring a new enum value with a data value less than the previous will end a block and
    ...
    /** This constructor makes a BlockType based on the previous value's I.D. and the given data. If the previous value's data value is <= <b><tt>data</b></tt>, then the I.D.
     ...
     * multiple enum constants that all have the same I.D., but different data values) are delimited by the use of the {@link #BlockType(int)} and {@link #BlockType(int, int)}
     * constructors; declaring a new enum value with a data value less than the previous will end a block and declaring one with a data value >= 0 will start a new block.
     * 
     * @param data
    ...
    /** This method determines whether or not a block of this {@link BlockType} can stop grass from spreading onto nearby dirt if placed on top of the dirt. For example, if you
     * place {@link BlockType#STONE stone} on top of dirt, that dirt cannot grow grass even if there is well-lit grass right next to it; on the other hand,
     ...
     * {@link BlockType#GRASS grass} and {@link BlockType#STONE stone}; others like {@link BlockType#GLASS_PANE glass panes} and {@link BlockType#ANVIL anvils} don't take on a
     * perfectly 1x1x1 cubic shape.
     * 
     ...
     * lamps} will emit lots of light (light level 1) and others like {@link BlockType#POWERED_REDSTONE_TORCH powered redstone torches} can emit some low light (light level 0.5
     * in this case).
     * 
    ...
    public Color getMapColor() {
        /* Mojang decided that they don't need to use three values for a color, but one big int in which the bits were basically concatenated from the three bytes representing
         * red, green, and blue values; this splits the int into three RGB values and puts it into a standard Color object */
    ...
    /** This method determines whether or not a block of this {@link BlockType} is able to be burned until broken. Note that most blocks can be lit on fire, but that does not
     ...
     * {@link BlockType#BEDROCK bedrock} cannot be pushed by pistons while {@link BlockType#STONE stone} blocks can be pushed easily and {@link BlockType#TORCH torches} can be
     * pushed, but will break and not push the block after them.
     * 
    ...
    /** This method determines whether or not a block of this {@link BlockType} is liquid. So far (as of 1.8), {@link BlockType#WATER water}, {@link BlockType#STATIONARY_WATER
     * stationary water}, {@link BlockType#LAVA lava}, and {@link BlockType#STATIONARY_LAVA stationary lava} are the only liquid blocks in Minecraft.
     * 
    ...
    /** This method determines whether or not a block is able to be pushed by a piston (or pulled by a sticky piston) without being broken. For example, {@link BlockType#STONE
     * stone} blocks are easily pushed without breaking while {@link BlockType#TORCH torches} can be pushed, but break off when they are pushed, and {@link BlockType#BEDROCK
     * bedrock} cannot be pushed at all.
     * 
    ...
    /** This method determines whether or not a block of this {@link BlockType} is "replaceable", meaning a new block can be placed where this block is without breaking this
     * block first. This would return <b>true</b> for things like {@link BlockType#TALL_GRASS tall grass} and {@link BlockType#WATER water}, but not for things like
     ...
     * blocks}, {@link BlockType#STONE stone}, and {@link BlockType#ANVIL anvils} are solid, but {@link BlockType#TALL_GRASS tall grass}, {@link BlockType#SIGN_POST signs}, and
     * {@link BlockType#WATER water} are not.
     * 
     ...
     * {@link BlockType#STONE stone} blocks will not drop anything unless a tool (in this case, a pickaxe) is used to break it; on the other hand, {@link BlockType#DIRT dirt}
     * can drop dirt even if a player uses their bare hands to dig it out.
     * 
