...
/** This enum is used to represent the different types of {@link Item}s. This list of different types not only includes those types of items differentiated by different I.D.s,
 * but also many of those differentiated by different data values; for example, all different colors of dye are listed individually despite the fact that they all have the
 * same I.D. */
public class ItemType extends HoldableType<ItemType> {
    public static final ItemType IRON_SHOVEL = new ItemType(256, -1),
            ...
            INVISIBILITY_POTION = new ItemType(), REGENERATION_POTION_II = new ItemType(8225), SWIFTNESS_POTION_II = new ItemType(), POISON_POTION_II = new ItemType(8228),
            HEALING_POTION_II = new ItemType(), STRENGTH_POTION_II = new ItemType(8233), LEAPING_POTION_II = new ItemType(8235),
            ...
            // banners
            BLACK_BANNER = new ItemType(0), RED_BANNER = new ItemType(), DARK_GREEN_BANNER = new ItemType(), BROWN_BANNER = new ItemType(), DARK_BLUE_BANNER = new ItemType(),
            PURPLE_BANNER = new ItemType(), CYAN_BANNER = new ItemType(), LIGHT_GRAY_BANNER = new ItemType(), DARK_GRAY_BANNER = new ItemType(), PINK_BANNER = new ItemType(),
            LIGHT_GREEN_BANNER = new ItemType(), YELLOW_BANNER = new ItemType(), LIGHT_BLUE_BANNER = new ItemType(),
            MAGENTA_BANNER = new ItemType(),
    ...
    // constructors
    /** This constructor makes a ItemType based on the previous value's I.D. and data. If the previous value has no strictly associated data value (data value = -1), it means
     * that it has no sub-types (e.g. the different colors of wool or types of wood), so use the next I.D.; if it has a data value, give this ItemType the same I.D. and the
     ...
     * {@link #ItemType(int)} and {@link #ItemType(int, int)} constructors; declaring a new enum value with a data value less than the previous will end a block and declaring
     * one with a data value >= 0 will start a new block. */
    private ItemType() {
        super();
    ...
    /** This constructor makes a ItemType based on the previous value's I.D. and the given data. If the previous value's data value is <= <b><tt>data</b></tt>, then the I.D.
     ...
     * multiple enum constants that all have the same I.D., but different data values) are delimited by the use of the {@link #ItemType(int)} and {@link #ItemType(int, int)}
    ...
    private static HoldableType<?> fromMCToolMaterial(ToolMaterial material) {
        switch (material) {
            ...
            default:
                throw new CIE("This tool is made of a material that I don't recognize! Is Corundum running with a Minecraft server version higher than " + Corundum.MCVERSION,
                        "unidentified tool material", "material=" + material.toString());
        }
    }

    // class-specific methods
    /** This method returns the maximum amount of damage an {@link Item} of this {@link ItemType} can take before it breaks. {@link ItemType}s that represent items that do not
     * take damage such as {@link ItemType#STICK sticks} will return 0.
     * 
    ...
    /** This method determines whether or not this {@link ItemType} represents a potion like {@link ItemType#SWIFTNESS_POTION a Swiftness Potion}. Note that despite the fact
     * that they have the same I.D. as potions, this method does <i>not</i> return <b>true</b> for {@link ItemType#WATER_BOTTLE water bottles}.
     * 
    ...
    public ItemType[] getSiblings() {
        ...
        while (first_sibling_ordinal > 0 && values()[first_sibling_ordinal - 1].id == id)
            first_sibling_ordinal--;
        while (last_sibling_ordinal_plus1 <= values().length && values()[last_sibling_ordinal_plus1].id == id)
            last_sibling_ordinal_plus1++;

        // create an array of the appropriate size
        ItemType[] results = new ItemType[last_sibling_ordinal_plus1 - first_sibling_ordinal];

        /* fill in the array with the contents between first_sibling_ordinal and lasT_sibling_ordinal_plus1 (including the ItemType at ordinal first_sibling_ordinal, but NOT
         * including the ItemType at ordinal last_sibling_ordinal_plus1) */
